...
import { getDevice, updateDevice } from "../../redux/action/device";
...
const Update = () => {
  ...
  const [type = device?.type, setType] = useState();
  const [name = device?.name, setName] = useState();
  const [image, setImage] = useState();
  const [price = device?.price, setPrice] = useState();
